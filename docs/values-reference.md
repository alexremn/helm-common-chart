...
| `global.persistence.retain` | bool | `false` | Protect standalone PVCs from deletion: emits `helm.sh/resource-policy: keep`, plus `argocd.argoproj.io/sync-options: Prune=false` under `deployTool: argocd` (where `resource-policy` alone is inert, since ArgoCD never runs `helm install`/`upgrade`). **PVC-scoped only**: under `deployTool: argocd`, `PriorityClass` and `ClusterRole`/`ClusterRoleBinding` also carry `Prune=false`, but unconditionally — independent of this flag, since those are cluster-scoped objects that can be shared across Applications. It is **not opt-out-able** there: a conflicting `sync-options` value in a resource's own `annotations` is merged into the union, not substituted, so the chart's `Prune=false` always survives — deliberately asymmetric with `helm.sh/resource-policy: keep`, which a resource's own annotations *can* override. |
